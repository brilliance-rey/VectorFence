...
u8 LAN8720_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOE, ENABLE);//使能GPIO时钟 RMII接口
...
//	  ETH_RESET-------------------------> PD3
	  ...
	  PA1（PIN_24）		O	RMII_REF_CLK	50M参考时钟
	  PA2（PIN_25）		I/O	ETH_MDIO		以太网管理数据
	  PA6（PIN_31）		I	RMII_RX_ERR		接收错误指示
	  PA7（PIN_32）		O	RMII_CRS_DV		接收数据有效
	  PC1（PIN_16）		O	ETH_MDC			以太网管理时钟
	  PC4（PIN_33）		I	RMII_RXD0		以太网接收数据bit0
	  PC5（PIN_34）		I	RMII_RXD1		以太网接收数据bit1
	  PE2（PIN_1）		O	ETH RESET		以太网芯片复位
	  PB11（PIN_48）	O	RMII_TX_EN		以太网发送使能
	  PB12（PIN_51）	O	RMII_TXD0		以太网发送数据bit0
	  PB13（PIN_52）	O	RMII_TXD1		以太网发送数据bit1
...
*/
	...
	GPIO_PinAFConfig(GPIOA, GPIO_PinSource6, GPIO_AF_ETH);
	...
	delay_ms(50);	
	LAN8720_RST=1;				//复位结束 
	ETHERNET_NVICConfiguration();//配置ETH中的分组
	rval=ETH_MACDMA_Config();  //1:成功
...
u8 ETH_Reinit(void){
/* Configure Ethernet */
	u8 rval=ETH_Init(&ETH_InitStructure, LAN8720_PHY_ADDRESS);		//配置ETH
...
* 1: 连接， 0：断开。
**/
u8 ETH_Get_Link_Status(void){
	if(ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, PHY_BSR) & (PHY_AutoNego_Complete | PHY_Linked_Status)){/*检测到网线连接*/
		return 1;
	}else{
		return 0;
	}
...
//	speed=((ETH_ReadPHYRegister(0x00,31)&0x1C)>>2); //从LAN8720的31号寄存器中读取网络速度和双工模式
	speed=((ETH_ReadPHYRegister(LAN8720_PHY_ADDRESS, PHY_SR) & ( PHY_SPEED_STATUS | PHY_DUPLEX_STATUS ))>>2); //从LAN8720的31号寄存器中读取网络速度和双工模式
	return speed;
}
/////////////////////////////////////////////////////////////////////////////////////////////////
...
//		ETH_DMAITConfig(ETH_DMA_IT_NIS|ETH_DMA_IT_R,ENABLE);  	//使能以太网接收中断	
//	}
	//ETH_SUCCESS            ((uint32_t)1)
	return rval; // 首次返回失败1， 初始化不 //配置ETH, 再lwip_comm.c中的Eth_Link_ITHandler()中ETH_Reinit();
}
