...
//					lwipdev.remoteip[3], loca_port);
			while (1) {
				if ((tcp_client_flag & LWIP_SEND_DATA) == LWIP_SEND_DATA){ //有数据要发送
					err = netconn_write(tcp_clientconn, tcp_client_sendbuf, strlen((char*) tcp_client_sendbuf),
...
//					printf("%s\r\n", tcp_client_recvbuf);
					...
					sprintf((char *)rcvd, "%s",tcp_client_recvbuf);  //通过串口发送接收到的数据
					rcvd[data_len] = '\0';
					data_len=0;  //复制完成后data_len要清零。

					if(strcmp(rcvd, "654321") == 0){
